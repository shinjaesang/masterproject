...
<body>
                ...
                <div class="row g-4">
                    <div class="col-12">
                        <div class="bg-light rounded h-100 p-4">
                            <h3 class="mb-4">출고전표생성</h3>
                            
                            <div class="form-container">
                                <form id="inoutForm" action="${pageContext.request.contextPath}/inout/outregister.do" method="post">
                                    <div class="row mb-4">
                                        <div class="col-md-3">
                                            <label for="outWarehouseId" class="form-label">출고 창고 선택</label>
                                        </div>
                                        <div class="col-md-9">
                                            <select id="outWarehouseId" name="outWarehouseId" class="form-select" required>
                                                <option value="">출고 창고를 선택하세요</option>
                                    ...
                                    <div class="row mb-4" id="inWarehouseDiv" style="display: none;">
                                        <div class="col-md-3">
                                            <label for="inWarehouseId" class="form-label">입고 창고 선택</label>
                                        </div>
                                        <div class="col-md-9">
                                            <select id="inWarehouseId" name="inWarehouseId" class="form-select">
    ...
    <script>
        ...
        document.getElementById('inoutForm').addEventListener('submit', function(e) {
            ...
            const outWarehouseId = document.getElementById('outWarehouseId').value;
            ...
            // 이동인 경우 입고창고 필수 체크
            if (inoutvoiceType === '이동') {
                const inWarehouseId = document.getElementById('inWarehouseId').value;
                if (!inWarehouseId) {
        ...
        document.getElementById('inoutvoiceType').addEventListener('change', function() {
            ...
            const inWarehouseDiv = document.getElementById('inWarehouseDiv');
            const inWarehouseId = document.getElementById('inWarehouseId');
            const type = this.value;
            
            if (!inoutvoiceName.value) {
                switch(type) {
                    case '이동':
                        inoutvoiceName.placeholder = '이동 전표제목을 입력하세요';
                        inWarehouseDiv.style.display = 'flex';
                        inWarehouseId.required = true;
                        break;
                    case '출고':
                        inoutvoiceName.placeholder = '출고 전표제목을 입력하세요';
                        inWarehouseDiv.style.display = 'none';
                        inWarehouseId.required = false;
                        break;
                    default:
